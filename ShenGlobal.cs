...
namespace Shenlong
{
	...
	public class ShenGlobal
	{
		...
		public static bool BuildQueryColumnSQL(XmlDocument xmlShenlongColumn, Dictionary<string, string> selectParams, /*bool checkHeaderOutput, int maxRowNum, */out string buildedSql, out string columnComments, ref List<string> fromTableNames, int indentCnt)
		{
			...
			bool checkHeaderOutput = false;
			int maxRowNum = -1;

			try
			{
				LogMessage(MethodBase.GetCurrentMethod().Name, null, mout.strb);
				...
				foreach ( XmlNode column in xmlShenlongColumn.DocumentElement.SelectNodes(tagColumn) )
				{
					...
					if ( (expression.Length != 0)/**/ && !string.IsNullOrEmpty(value1)/**/ )
					{
						...
						SetShenlongParam(selectParams, xmlShenlongColumn.BaseURI, bubbles, tableName + "." + plainFieldName/*plainTableFieldName*/, ref paramNames, ref expression, ref value1, ref value2);
				...
				foreach ( string subQuery in _fileSubQuery )
				{
					if ( fromJoins != null )
					{
						...
						int j;
						for ( j = 0; (j < fromJoins.Count) && (_subQuery != GetTableName(fromJoins[j].tableName, true)); j++ ) ;
				...
				if ( defWhereLen < where.Length )
				{
					...
					{
						// OR 条件を閉じ括弧でターミネートする
						TerminateOrRoundBlanket(ref cameOR, lastSpace++, ref where);
					}
					if ( !_fileUseJoin )
					{
			...
			{
				LogMessage(exp.Message, MethodBase.GetCurrentMethod().Name, mout.all);
				return false;
			}
		}
		...
		public static List<string> GetTableNameInSQL(string sql, bool plain, bool distinct, out int untilFromIndex)
		{
			...
			try
			{
				...
				if ( sql.IndexOf(" JOIN", startTableName, StringComparison.CurrentCultureIgnoreCase) == -1 )
				{
					for ( int i = startTableName; i < sql.Length; i++ )
					{
						...
						else if ( (sql[i] == '\r') || (sql[i] == '\n') )
						{
							continue;
						}
						else if ( sql[i] == ' ' )
						{
							for ( ; (i + 1 < sql.Length) && (sql[i + 1] == ' '); i++ ) ;	// 連続した空白であればスキップする
							tableName += sql[i].ToString();
							continue;
						}
						else if ( sql[i] == '(' )	// サブクエリ？
						{
				...
				{
					string[] beginOfJoin = { "JOIN", "INNER JOIN", "LEFT OUTER JOIN", "LEFT JOIN", "RIGHT OUTER JOIN", "RIGHT JOIN", "FULL OUTER JOIN" };
					string[] onAnd = { "ON ", "AND " };
					...
					for ( int i = startTableName; i < sql.Length; i++ )
					{
						...
						else if ( Char.IsLetter(sql[i]) )
						{
							...
							if ( (index = CompareAnyString(sql, i, beginOfJoin)) != -1 )
							{
								Debug.Write(sql.Substring(i + 1, beginOfJoin[index].Length));
								AddSqlTableName(ref tableNames, ref tableName, plain, distinct);
								i += beginOfJoin[index].Length;
								continue;
							}
							// ON|AND (～) ?
							else if ( (index = CompareAnyString(sql, i, onAnd)) != -1 )
							{
								Debug.Write(sql.Substring(i + 1, onAnd[index].Length - 1));
								AddSqlTableName(ref tableNames, ref tableName, plain, distinct);
								i += onAnd[index].Length;
								...
								{
									if ( (index = sql.IndexOf("=", i, sql.Length - i)) != -1 )
									{
										for ( index++; (index < sql.Length) && (sql[index] == ' '); index++ ) ;// 空白をスキップする
										Debug.Write(sql.Substring(i, index - i));
										i = index;
										// 右側の結合条件をスキップする
										for ( ; (i < sql.Length) && (Char.IsLetterOrDigit(sql[i]) || Char.IsPunctuation(sql[i])); i++ ) ;
										Debug.Write(sql.Substring(index, i - index));
									}
									i--;
								}
								continue;
							...
							// FROM の終わり？
							else if ( CompareAnyString(sql, i, endOfFrom) != -1 )
							{
								AddSqlTableName(ref tableNames, ref tableName, plain, distinct);
								untilFromIndex = i - 1;
								break;
					...
					{
						string _tableName = tableName.TrimStart();
						if ( (_tableName.Length != 0) && (_tableName[0] != ')') )	// サブクエリ終端の右括弧ではない？
						{
							AddSqlTableName(ref tableNames, ref tableName, plain, distinct);
						}
					}
...
#if (DEBUG)
			log.AppendLine("[" + MethodBase.GetCurrentMethod().Name + "]");
			log.AppendLine(sql);
			log.AppendLine("plain:" + plain);
			log.AppendLine("distinct:" + distinct);
			foreach ( string tableName in tableNames )
			{
				log.AppendLine(tableName);
			}
			log.AppendLine("untilFromIndex: " + untilFromIndex);
			log.AppendLine("");
		...
		public static void WriteAccessLog(string[] writeLogDsnUidPwd, string serviceName, string userName, List<string> tableNames, pno progNo)
		{
			...
			try
			{
				...
				string conStr = "Provider=MSDAORA;" +
								"Data Source=" + writeLogDsnUidPwd[0]/*infoPubSID*/ + ";" +
								"user id=" + writeLogDsnUidPwd[1]/*infoPubUser*/ + ";" +
								"password=" + writeLogDsnUidPwd[2]/*infoPubPwd*/ + ";" +
				...
				foreach ( string tableName in tableNames )
				{
					string sql = "INSERT INTO T_LOG_SHENLONG " +
